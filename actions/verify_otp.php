...
try {
    ...
    if (!$email || !$phone || !$password || !$role || !$linkedin) {
        ob_clean();
    ...
    // Insert user
    $stmt = $conn->prepare("
        INSERT INTO users (email, phone, password, role, linkedin_url, is_verified)
        VALUES (?, ?, ?, ?, ?, 1)
    ");
    if (!$stmt) {
        throw new Exception("DB Error (insert): " . $conn->error);
    }
    $stmt->bind_param("sssss", $email, $phone, $hashed_password, $role, $linkedin);
